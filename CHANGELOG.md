...
- Improved how masks are parsed in CanvasRenderer, and how window resize is handled.
...
- Improved preloading om assets, fixing flicker on some browser for asset heavy animations.
- Fixed issue with how masks are rendered in CanvasRenderer.
- Improved DPR detection in CanvasRenderer.

## [6.4.0] - 06-08-2026

### Changed

- Major change in how critical load error are handled – from being relegated to console logs while staying quiet on the front end, to rendering graphic error screen. This behavior can be changed by using the boolean `quiet` attribute.
